...
type CreateTagScrema = z.infer<typeof createTagSchema>;
...
export function CreateTagForm() {
  ...
  const { register, handleSubmit, watch, formState } = useForm<CreateTagScrema>(
    {
  ...
  const { mutateAsync } = useMutation({
    mutationFn: async ({ title }: CreateTagScrema) => {
      // Delay 2s
  ...
  async function createTag({ title }: CreateTagScrema) {
